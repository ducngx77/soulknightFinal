...
public class RoomFirstDungeonGenerator : SimpleRandomWalkDungeonGenerator
{
    ...
    [SerializeField]
    private List<GameObject> agentPrefab;
    [SerializeField]
    ...
    public void CreateRooms()
    {
        ...
        for (int i = 0; i < roomsList.Count; i++)
        {
            ...
            if (i == 0)
            {
                ...
                if (player == null)
                {
                    Instantiate(GetCharSelected(), new Vector3(firstRoomCenter.x, firstRoomCenter.y, 0) , Quaternion.identity);
                } else
                {
    ...
    {
        GameObject player = GameObject.FindGameObjectWithTag("Player");
        if (player.GetComponent<CoinCount>().level == 1)
        {
            return chess[0];
        } else
        {
